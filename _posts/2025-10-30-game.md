...
> Manifest：热更新/资源加载的“说明书”，记录每个包或资源的路径、版本号、依赖关系和校验信息，客户端根据它决定要下载哪些文件、存哪、如何校验完整性。打包后 Cocos Creator 会产出 project.manifest 或 version.manifest，里面写明 cdnVersion、主包/分包清单等。

> MD5：一种常用的哈希算法，把文件内容映射成固定长度的校验值（通常表现为 32 位十六进制字符串）。构建时会计算文件的 MD5 并写进 manifest，这样客户端下载后再次计算 MD5，比较两个值就能确认文件是否损坏或被篡改，也能利用 MD5 变化判断资源是否更新。